...
MyRigidBody::MyRigidBody(std::vector<vector3> a_pointList)
{
	...
	for (uint i = 1; i < points; i++) // set max and min values
	{
	...
	vector3 v3Size;
	v3Size = m_v3MaxL - m_v3MinL; // measure x, y, z
	*/

	m_v3HalfWidth = (m_v3MaxL - m_v3MinL) / 2.0f; // gives measurement of center to each axis
...
bool MyRigidBody::IsColliding(MyRigidBody* const other)
{
	...
	//return (glm::distance(m_v3Center, other->m_v3Center) < (m_fRadius + other->m_fRadius)); // more efficent than above
}